...
namespace LAB3.Classes
{
    class Student
    {
        ...
        public static List<Student> addStudentsFromFile(string fileName)
        {
            // listai studentam is failo
            List<Student> studentsList = new List<Student>();
            List<int> studentGrades = new List<int>();
            ...
            {
                // pakeiti path i failo, kuri norite kad uploadintu
                StreamReader fileReader = new StreamReader("C:\\Users\\adoma\\source\\repos\\LAB3\\LAB3\\results\\" + fileName);
                bool skipFirst = true;
                while (!fileReader.EndOfStream)
                {
                    var line = fileReader.ReadLine(); var splitOn = line.Split(',');
        ...
        public static void generateStudentAmount(int amount)
        {
            StreamWriter studentsAmount = new StreamWriter($"C:\\Users\\adoma\\source\\repos\\LAB3\\LAB3\\results\\{amount}_students.csv");

            studentsAmount.WriteLine("Name, Surname, HW1, HW2, HW3, HW4, HW5, Exam");
            for (int i = 0; i < amount; i++)
                studentsAmount.WriteLine(new Student().csvToString());

            studentsAmount.Close();
        ...
        // converts csv to string so it can be easily written to file, overloaded function
        ...
        {
            var lineToWrite = $"{Name},{Surname},";
            foreach (var item in HomeworkGrades) { lineToWrite += $"{item},"; }
            lineToWrite += $"{ExamGrade}";
            return lineToWrite;
        }

        public static IEnumerable<Student> chooseContainerType(string containerType, string file)
        {
            if (containerType.Equals("list"))
            {
                return new List<Student>(Student.addStudentsFromFile(file));
            }
            else if (containerType.Equals("linkedlist"))
            {
                return new LinkedList<Student>(Student.addStudentsFromFile(file));
            }
            else if (containerType.Equals("queue"))
            {
                return new Queue<Student>(Student.addStudentsFromFile(file));
            }
            else
            {
                return null;
        ...
        public static void measurePerformance(string type, string filename, int amount)
        {

            Stopwatch timer = new Stopwatch();
            timer.Restart();

            var students = chooseContainerType(type, filename);

            StreamWriter passedList = new StreamWriter($"C:\\Users\\adoma\\source\\repos\\LAB3\\LAB3\\results\\passed_{amount}_students.csv");
            StreamWriter failedList = new StreamWriter($"C:\\Users\\adoma\\source\\repos\\LAB3\\LAB3\\results\\failed_{amount}_students.csv");

            foreach (Student student in students)
            {
                if (student.calculateFinalResult() >= 5)
                {
                    passedList.WriteLine(student.csvToString());
                }
                else
                {
                    failedList.WriteLine(student.csvToString());
                }
            }
            ...
            timer.Stop();
            Console.WriteLine($"Type : {students.GetType().Name} | Sorted: {amount} | Time elapsed: {timer.Elapsed}");
        ...
        public static void testContainers()
        {
            ...
            measurePerformance("list", @"/10000_students.csv", 1000);
            measurePerformance("linkedlist", @"/10000_students.csv", 1000);
            measurePerformance("queue", @"/10000_students.csv", 1000);
            Console.WriteLine();
            measurePerformance("list", @"/100000_students.csv", 10000);
            measurePerformance("linkedlist", @"/100000_students.csv", 10000);
            measurePerformance("queue", @"/100000_students.csv", 10000);
            Console.WriteLine();
            measurePerformance("list", @"/1000000_students.csv", 100000);
            measurePerformance("linkedlist", @"/1000000_students.csv", 100000);
            measurePerformance("queue", @"/1000000_students.csv", 100000);
            Console.WriteLine();
            measurePerformance("list", @"/10000000_students.csv", 1000000);
            measurePerformance("linkedlist", @"/10000000_students.csv", 1000000);
            measurePerformance("queue", @"/10000000_students.csv", 1000000);
            Console.WriteLine();
        }
